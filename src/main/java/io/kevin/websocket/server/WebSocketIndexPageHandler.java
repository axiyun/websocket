 ...
 * under the License.
 */
package io.kevin.websocket.server;
...
public class WebSocketIndexPageHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest req) throws Exception {
        ...
        if ("/".equals(req.uri()) || "/index.html".equals(req.uri())) {
            ...
            FullHttpResponse response = new DefaultFullHttpResponse(req.protocolVersion(), HttpResponseStatus.PERMANENT_REDIRECT); //设置重定向响应码 （临时重定向、永久重定向）
            ...
            headers.set(HttpHeaderNames.LOCATION, "http://localhost:8080"); //重定向URL设置
            ctx.writeAndFlush(response).addListener(ChannelFutureListener.CLOSE);//解决写入完成后，客户端断开会报异常的问题
